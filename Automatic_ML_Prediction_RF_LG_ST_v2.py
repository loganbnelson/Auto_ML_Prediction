...
# Let's set up a function to split data if only one file is submitted. It must contain all Y output values througout.
...
def select_or_create_column(dataframe, prompt="\n\nSelect the appropriate column as index. If no column represents the index just hit Enter:  ", default_column=None):
    """
    ...
    print("\n\nAvailable columns to select for the index column:")
...
# Enter the column that is the output of the training and test set.

data_split = split_data(train_data, test_data)

X_test_encoded_index = select_or_create_column(data_split[2], default_column=None)
